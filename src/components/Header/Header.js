...
const Header = () => {
	...
	return (
		<header className='text-gray-600 body-font'>
			<Modal title='Authenticate' open={isLoginModalOpen} handleClose={handleCloseLoginModal}>
				...
				<div className='flex flex-col items-center justify-center h-80'>
					<button
						type='button'
						onClick={loginStep === 1 ? handleConnectWalletClick : handleAnonAuthClick}
						className={`px-2 py-2 ${
							loginStep === 1
								? 'bg-white'
						...
						<div>
							<Menu.Button className='flex flex-row items-center justify-between w-full border-[0.5px] rounded-full p-1'>
								<img
									...
									alt=''
								/>
								<p className='text-ellipsis overflow-hidden w-40 px-4'>{address}</p>
							</Menu.Button>
						</div>
						<Transition
